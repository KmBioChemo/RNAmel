...
- Saving a project now captures the full session – transcription-factor
  / pathway activity, GSVA signatures and the AI interpretation – not
  only enrichment and WGCNA; restoring a project brings every slot back.
- The all-pairwise shrinkage checkbox no longer labels its estimator as
  `apeglm` (that path uses `normal` shrinkage; the estimator actually
  used is recorded per contrast).
...
- The normalization method actually used (VST, or the `log2(counts + 1)`
  fallback when VST fails) is recorded in the saved project and surfaced
  in the Methods text; the fallback notification is now persistent.
- The Methods paragraph no longer describes every contrast with the
  first contrast’s parameters, and states that reported software
...
- Corrected the TCGA demonstration-dataset citation (Rahman *et al.*
  2015).
- Added `THIRD_PARTY_NOTICES.md` (airway, TCGA / GSE62944, CollecTRI,
...
- `R-CMD-check` now runs on pushes to `main` and across an R version
