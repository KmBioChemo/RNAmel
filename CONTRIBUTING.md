...
- **Bug report** — include a minimal reproducible example (a small counts +
  metadata snippet), the exact error, and `sessionInfo()`.
- **Feature request** — describe the analysis or UI need and, if possible, the
  intended workflow.
...
bundled `Dockerfile` pins a known-good platform (R 4.5 / Bioconductor 3.22) if
you would rather not manage the dependencies locally.
...
- **Shiny modules** — `mod_*` files are thin UI + server wrappers that read
  inputs and call the pure layer.
