...
#' reproduces the pipeline with AMEL's public functions -- ready to drop
...
generate_r_script <- function(project,
  ...
  add(sprintf("# AMEL reproducible analysis script"),
  ...
  if (length(store) == 0) {
    ...
    for (i in seq_along(store)) {
      ...
      if (is_computed) {
        ...
        # Uploaded DE table: AMEL cannot recompute it. Define the object via
        # a real (user-completed) read call so downstream steps stay valid.
        add(sprintf("# %s (uploaded DE table -- AMEL cannot recompute it)", lbl),
  ...
  # Anchor the figure/enrichment examples on a contrast AMEL actually
