...
#'
#' Builds a self-contained HTML report of an AMEL session using
...
session_manifest <- function() {
  pkgs <- c("AMEL", "DESeq2", "SummarizedExperiment", "ggplot2", "fgsea",
...
#' Embed any AMEL figure as an <img> data URI
#' @keywords internal
...
#' @export
build_report_html <- function(project, file, title = "AMEL analysis report",
                              generated = NULL) {
  organism <- project$organism %||% "unknown"
  store    <- project$contrasts %||% list()
  counts   <- project$counts
  meta     <- project$metadata
  ver <- tryCatch(as.character(utils::packageVersion("AMEL")),
  ...
  doc <- htmltools::tagList(
    ...
    h$div(
      ...
      h$div(class = "rh-sub",
            sprintf("Generated by AMEL v%s%s", ver,
    ...
    section(h$h2("Reproducible R script"),
            h$p(class = "muted",
                "A script that reproduces the pipeline with AMEL (with the ",
    ...
    section(h$h2("Session"),
            h$p(h$span(class = "badge", paste0("AMEL ", ver)),
